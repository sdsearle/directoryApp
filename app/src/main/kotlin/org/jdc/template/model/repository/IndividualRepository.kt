...
@Inject constructor(
    ...
    suspend fun saveIndividual(individual: Individual) {
        ...
        if (individual.id <= 0 && !exists(individual)) {
    ...
    private suspend fun exists(individual: Individual): Boolean {
        for(entry in individualDao().findAll()){
            if(entry.isSame(individual)){
                return true
            }
        }
        return false
